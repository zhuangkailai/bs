...
public class JPushClientUtil {
    ...
    /**
     * @author: lip
     * @date: 2019/8/2 0002 下午 4:28
     * @Description: 给安卓平台指定用户发送内容消息
     * @updateUser:
     * @updateDate:
     * @updateDescription:
     * @param:
     * @return:
     */
    public void sendMessageToAppointAndroid(String regId, String contenttype, String msgContent) {
     ...
     * @author: lip
     * @date: 2019/8/2 0002 下午 4:28
     * @Description: 给安卓平台指定用户发送内容消息和通知消息
     * @updateUser:
     * @updateDate:
     * @updateDescription:
     * @param:
     * @return:
     */
    public void sendMessageAndTitleToAppointAndroid(String regId,
     ...
     * @return:
     */
    public void sendTaskSMS(String phoneNumber, int smsTempId, String username, String time, String mointorpointname, String tasktype) throws Exception {
        if (isPhone(phoneNumber)) {
            SMSPayload payload = SMSPayload.newBuilder()
                    .setMobileNumber(phoneNumber) // 手机号码
                    .setTempId(smsTempId)            // 短信模板ID,需要自己去申请
                    .addTempPara("username", username)  // 短信内容
                    .addTempPara("time", time)
                    .addTempPara("mointorpointname", mointorpointname)
                    .addTempPara("tasktype", tasktype)
                    .build();
            SendSMSResult res =  this.getSMSClient().sendTemplateSMS(payload);
        }
    }
     ...
     * @return:
     */
    public void sendPasswordSMS(String phoneNumber, int smsTempId, String code) throws Exception {
        if (isPhone(phoneNumber)) {
            SMSPayload payload = SMSPayload.newBuilder()
                    .setMobileNumber(phoneNumber) // 手机号码
                    .setTempId(smsTempId)            // 短信模板ID,需要自己去申请
                    .addTempPara("password", code)  // 短信内容
                    .build();
            SendSMSResult res =  this.getSMSClient().sendTemplateSMS(payload);
        }
    }
     ...
     * @return:
    */
    public void sendAlarmSMS(Object messageParam, int smsTempId) throws APIConnectionException, APIRequestException {

        JSONObject jsonObject = JSONObject.fromObject(messageParam);


        List<String> phoneNumbers = jsonObject.getJSONArray("phoneNumbers");
        String entName = jsonObject.getString("entName");
        String monitorPointName = jsonObject.getString("monitorPointName");
        String monitorTime = jsonObject.getString("monitorTime");
        String overContent = jsonObject.getString("overContent");;
        for (String phoneNumber:phoneNumbers){
            if (isPhone(phoneNumber)) {
                SMSPayload payload = SMSPayload.newBuilder()
                        ...
                        .setTempId(smsTempId)
                        // 短信模板ID,需要自己去申请
                        .addTempPara("entname", entName)  // 企业名称
                        .addTempPara("monitorpointname", monitorPointName)  // 点位名称
                        .addTempPara("monitortime", monitorTime)  // 报警时间
                        .addTempPara("overcontent", overContent)  // 报警内容
                        .build();
                SendSMSResult res =  this.getSMSClient().sendTemplateSMS(payload);
            }
        }
     ...
     * @updateDate:
     * @updateDescription:
     * @param: entname,monitortime
     * @return:
    */
    public boolean sendOffSMS(String phoneNumber, Map<String,String> paramSMS, int smsTempId) throws APIConnectionException, APIRequestException {
       boolean resString = false;
        if (isPhone(phoneNumber)) {
            SMSPayload payload = SMSPayload.newBuilder()
                    .setMobileNumber(phoneNumber) // 手机号码
                    .setTempId(smsTempId)// 短信模板ID,需要自己去申请
                    .setTempPara(paramSMS)//动态参数
                    .build();
            SendSMSResult res =  this.getSMSClient().sendTemplateSMS(payload);
            resString = res.isResultOK();
        }
        return resString;
    }
}
